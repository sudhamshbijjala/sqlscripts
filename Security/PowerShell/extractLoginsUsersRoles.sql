--  Script: User and permission restoring script
--  Author: Richard Ding
--  Created: 7/2/2009
--  Modified: 10/06/2009
...
--select * from tempdb.dbo.UserAccountToKeep order by name asc;
...
exec (@string)
  ...
  update tempdb.dbo.RoleMemberMapping set Script = 
    case when RoleType = 'DB Role' then 'EXEC dbo.sp_addrolemember ''' + RoleName + ''', ''' + RoleMember + ''';'
    else Script end
  where Script is null

--select * from tempdb.dbo.RoleMemberMapping order by MappingId asc, RoleName asc, RoleMember asc
...
when Scope like '%DATABASE' OR Scope like '%SERVER' OR Scope like '%ENDPOINT' then 'USE [' + @dbname + ']; ' + [Status] + ' ' + Permission + ' TO ' + quotename(Grantee, '[') + ';'
END

--select * from tempdb.dbo.UserPermission ORDER BY UserPermissionId asc
